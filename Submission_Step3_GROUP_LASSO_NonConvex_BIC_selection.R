#########
## Variable selection via Non-Convex Penalties: SCAD & MCP
...
for (criter in all.criters){
  ...
  for (year in years){
    ...
    all.gammas <-  read.csv(file=paste0(getwd(),"/GROUP_LASSO/NonConvex/",
                                        penalty, "_",
                                        "Gammas_",
                                        ifelse(IMPUTE.NONMAJOR.NA, "IMPUTED_", ""),
    ...
    for (stat in main.stats){
      ...
      cv.lasso.obj <- cv.grpreg(x,y, 
                                group=group.ind-1,
                                gamma=as.numeric(all.gammas[which(main.stats == stat), 2]),
                                foldid=flds,
      ...
      n.obs <- cv.lasso.obj$fit$n
      df <- cv.lasso.obj$fit$df
      
      if (GLM_type == "Gaussian"){
        tLL <- n.obs*log(cv.lasso.obj$fit$loss)
      } else {
        tLL <- cv.lasso.obj$null.dev*n.obs - cv.lasso.obj$fit$loss
      }
